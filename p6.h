...
class binary_tree {
  node<T> root;
public:
  binary_tree(T data): root{make_node(data)} {}
  ...
  void bfs(UnaryFunction func) {
    ...
    while (q.empty() == false) {
      ...
      // result.push_back(value->data); // ----> TAREA ESPECIFICA 
...
private:
  bool are_siblings_(node<T> nodoA, node<T> nodoB);
};

template <typename T>
bool binary_tree<T>::are_siblings_(node<T> nodoA, node<T> nodoB){
    T countA = 0;
    T countB = 0;

    while(nodoA->parent != nullptr){
        nodoA = nodoA->parent;
        countA++;
    }

    while(nodoB->parent != nullptr){
        nodoB = nodoB->parent;
        countB++;
    }
